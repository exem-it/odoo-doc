...
class GenerateHtml:
    ...
    def generate_models_index(self, odoo, model_fields):
        for model in model_fields:
            ...
            pages_html = pages_html.replace(
                "</HERE CONTENT>",
                f"<h3><strong>404:</strong> This model as no field.</h3>\n"
            )
            os.makedirs(f"{self.output_folder}/{model_folder}")
            with open(f"{self.output_folder}/{model_folder}/index.html", "a+", encoding="utf-8") as f:
                f.write(pages_html)
